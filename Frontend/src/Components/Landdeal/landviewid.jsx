...
const LandView = () => {
  ...
  return (
    <>
      <Navbar />
      <div className="land-view-container">
        <div className="land-card">
          <div className="land-images-section">
            {land.photos && land.photos.length > 0 ? (
              <div className="land-image-grid">
                {land.photos.map((image, index) => (
                  <img
                    key={index}
                    src={`${config.apiURL}/${image}`}
                    alt={`Land Image ${index + 1}`}
                    className="land-image-item"
          ...
          <div className="land-details-section">
            <h2 className="land-title">{land.projectName || 'N/A'}</h2>
